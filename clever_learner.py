...
class Learner:
    ...
    def __init__(self, ota):
        ...
        # since the equlity of tw1[:-1] and tw2[:-1] should be checked after adding new suffix.
    ...
    def addSuffix(self, suffix):
        ...
        # check equality of tw1[:-1] and tw2[:-1] after adding 'e'
        # if they are different now, remove them from consistency constraints
        delete_items = []
    ...
    def addReset(self, rst):
        for r, v in rst.items():
            self.banned_reset.append(self.reset_name[r] != v)
    ...
    def findReset(self):
        ...
        for i in range(minimum_states_num, len(non_sink_R)+1):
            ...
            s = z3.Solver()
            s.add(*self.banned_reset, constraint1, constraint2, constraint4, constraint5, constraint6, constraint7, states_var[tuple()]==1)
            if str(s.check()) == "sat":
                result = "sat"
                break
            else:
...
def learn_ota(ota, limit=30, verbose=True):
    ...
    new_ctxs = set() # store the counterexamples which are found at the current level
    to_S = []
    ...
    for i in range(1, limit):
        ...
        if resets is None:
            ...
            # raise NotImplementedError
            for ctx_path in new_ctxs:
                learner.addPath(ctx_path)
            new_ctxs.clear()

            for s in to_S:
                learner.addPossibleS(s)

            learner.banned_reset.clear()
            to_S.clear()
            continue
